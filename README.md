...
This workflow DOES NOT generate submission ready SDTM datasets. Instead, it aims to make the SDTM dataset development process easier and more structured. The generated files can be used to support the development of SDTM datasets using tools such as `SAS` or `SDTM.OAK`.

The workflow starts with an eCRF containing SDTM annotations in the REDCap field annotations. These annotations are then used to generate SDTM mapping specifications and annotated CRFs.

*I personally recommend having an `Template eCRF`/`Mother eCRF` already mapped and revised to generate the subsequent eCRFs*

In addition, the `similarity based search` between `REDCap submission terms` and `SDTM Preferred Terms` can help facilitate subsequent development of SDTM datasets by creating a `Controlled Terminology File` that has some fields already populated.
